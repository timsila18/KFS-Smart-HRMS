...
class ReportDataService
{
    ...
    private function payrollLines(array $filters): Collection
    {
        return DB::table('payroll_run_items')
            ...
            ->when($filters['employer'] ?? null, fn ($q, $employer) => $q->where('employees.employer', $employer))
            ...
            ->get(['payroll_runs.run_number', 'payroll_periods.code as period', 'employees.employer', 'employees.employee_number', DB::raw("concat(employees.first_name,' ',employees.last_name) as employee"), 'pay_codes.code', 'pay_codes.name', 'pay_codes.pay_code_type', 'payroll_run_items.amount']);
    }
    ...
    private function employeeRegister(array $filters): Collection
    {
        return DB::table('employees')->leftJoin('stations', 'stations.id', '=', 'employees.station_id')->leftJoin('departments', 'departments.id', '=', 'employees.department_id')->leftJoin('job_positions', 'job_positions.id', '=', 'employees.job_position_id')
            ...
            ->get(['employees.employee_number', DB::raw("concat(employees.first_name,' ',employees.last_name) as employee"), 'employees.employer', 'employees.employment_status', 'stations.name as station', 'departments.name as department', 'job_positions.title as position', 'employees.hire_date']);
    }
    ...
    private function payCodeReport(string $code, array $filters): Collection
    {
        ...
        return DB::table('payroll_run_items')->join('pay_codes', 'pay_codes.id', '=', 'payroll_run_items.pay_code_id')->join('employees', 'employees.id', '=', 'payroll_run_items.employee_id')->join('payroll_runs', 'payroll_runs.id', '=', 'payroll_run_items.payroll_run_id')->leftJoin('payroll_periods', 'payroll_periods.id', '=', 'payroll_runs.payroll_period_id')
            ...
            ->get(['payroll_periods.code as period', 'employees.employer', 'employees.employee_number', DB::raw("concat(employees.first_name,' ',employees.last_name) as employee"), 'pay_codes.name', 'payroll_run_items.amount']);
    }

    private function p9(array $filters): Collection { return $this->payCodeReport('PAYE', $filters); }
    private function bankSchedule(array $filters): Collection { return DB::table('payslips')->join('employees','employees.id','=','payslips.employee_id')->join('payroll_runs','payroll_runs.id','=','payslips.payroll_run_id')->leftJoin('employee_bank_accounts','employee_bank_accounts.employee_id','=','employees.id')->when($filters['period_id'] ?? null, fn ($q, $id) => $q->where('payroll_runs.payroll_period_id', $id))->when($filters['department_id'] ?? null, fn ($q, $id) => $q->where('employees.department_id', $id))->when($filters['employer'] ?? null, fn ($q, $employer) => $q->where('employees.employer', $employer))->where('employee_bank_accounts.is_primary',true)->get(['employees.employer', 'employees.employee_number', DB::raw("concat(employees.first_name,' ',employees.last_name) as employee"), 'employee_bank_accounts.bank_code', 'employee_bank_accounts.bank_name', 'employee_bank_accounts.branch_code', 'employee_bank_accounts.branch_name', 'employee_bank_accounts.account_number', 'payslips.net_pay']); }
    ...
    private function hriskeIndividualPaymentBreakdown(array $filters): Collection { $rows = DB::table('payroll_runs')->join('payroll_run_items','payroll_run_items.payroll_run_id','=','payroll_runs.id')->join('pay_codes','pay_codes.id','=','payroll_run_items.pay_code_id')->join('employees','employees.id','=','payroll_run_items.employee_id')->leftJoin('employee_bank_accounts','employee_bank_accounts.employee_id','=','employees.id')->leftJoin('bank_branches','bank_branches.id','=','employee_bank_accounts.bank_branch_id')->leftJoin('payroll_periods','payroll_periods.id','=','payroll_runs.payroll_period_id')->when($filters['period_id'] ?? null, fn ($q, $id) => $q->where('payroll_runs.payroll_period_id',$id))->when($filters['department_id'] ?? null, fn ($q, $id) => $q->where('employees.department_id',$id))->when($filters['employer'] ?? null, fn ($q, $employer) => $q->where('employees.employer', $employer))->whereNull('payroll_run_items.deleted_at')->orderBy('employees.employee_number')->get(['payroll_periods.code as paymonth','employees.employer','employees.employee_number as payroll_num',DB::raw("concat(employees.first_name,' ',employees.last_name) as names"),'employee_bank_accounts.bank_name','employee_bank_accounts.branch_name','bank_branches.bank_code','bank_branches.branch_code','employee_bank_accounts.account_number','pay_codes.code','payroll_run_items.amount']); return $rows->groupBy('payroll_num')->map(function ($items) { $first = $items->first(); $gross = (float) $items->where('amount','>',0)->sum('amount'); $deductions = abs((float) $items->where('amount','<',0)->sum('amount')); $row = ['paymonth'=>$first->paymonth,'employer'=>$first->employer,'payroll_num'=>$first->payroll_num,'names'=>$first->names,'bank_name'=>$first->bank_name,'branch_name'=>$first->branch_name,'bank_code'=>$first->bank_code,'branch_code'=>$first->branch_code,'account_number'=>$first->account_number,'gross_pay'=>$gross,'total_deductions'=>$deductions,'net_pay'=>$gross - $deductions]; foreach ($items->sortBy('code') as $item) { $row[$item->code] = ($row[$item->code] ?? 0) + abs((float) $item->amount); } return (object) $row; })->values(); }
    private function hriskeBankSummary(array $filters): Collection { return DB::table('payslips')->join('payroll_runs','payroll_runs.id','=','payslips.payroll_run_id')->join('employees','employees.id','=','payslips.employee_id')->leftJoin('employee_bank_accounts','employee_bank_accounts.employee_id','=','employees.id')->when($filters['period_id'] ?? null, fn ($q, $id) => $q->where('payroll_runs.payroll_period_id',$id))->when($filters['employer'] ?? null, fn ($q, $employer) => $q->where('employees.employer', $employer))->where('employee_bank_accounts.is_primary',true)->selectRaw("count(distinct employees.id) as officers, coalesce(employee_bank_accounts.bank_code,'') as bank_code, coalesce(employee_bank_accounts.bank_name,'Unassigned') as banking_institution, sum(payslips.net_pay) as total_amount")->groupByRaw("coalesce(employee_bank_accounts.bank_code,''), coalesce(employee_bank_accounts.bank_name,'Unassigned')")->orderBy('banking_institution')->get(); }
    ...
    {
        return DB::table('payroll_run_items')->join('pay_codes','pay_codes.id','=','payroll_run_items.pay_code_id')->join('payroll_runs','payroll_runs.id','=','payroll_run_items.payroll_run_id')->join('employees','employees.id','=','payroll_run_items.employee_id')->leftJoin('payroll_periods','payroll_periods.id','=','payroll_runs.payroll_period_id')
    ...
    {
        return abs((float) DB::table('payroll_run_items')->join('pay_codes','pay_codes.id','=','payroll_run_items.pay_code_id')->join('payroll_runs','payroll_runs.id','=','payroll_run_items.payroll_run_id')->join('employees','employees.id','=','payroll_run_items.employee_id')->when($filters['period_id'] ?? null, fn ($q, $id) => $q->where('payroll_runs.payroll_period_id',$id))->when($filters['department_id'] ?? null, fn ($q, $id) => $q->where('employees.department_id',$id))->when($filters['employer'] ?? null, fn ($q, $employer) => $q->where('employees.employer', $employer))->where('pay_codes.component_subtype',$subtype)->whereNull('payroll_run_items.deleted_at')->sum('payroll_run_items.amount'));
    }

    private function officerCount(array $filters): int
    {
        return (int) DB::table('payroll_run_items')->join('payroll_runs','payroll_runs.id','=','payroll_run_items.payroll_run_id')->join('employees','employees.id','=','payroll_run_items.employee_id')->when($filters['period_id'] ?? null, fn ($q, $id) => $q->where('payroll_runs.payroll_period_id',$id))->when($filters['department_id'] ?? null, fn ($q, $id) => $q->where('employees.department_id',$id))->when($filters['employer'] ?? null, fn ($q, $employer) => $q->where('employees.employer', $employer))->whereNull('payroll_run_items.deleted_at')->distinct('employee_id')->count('employee_id');
    }
}
